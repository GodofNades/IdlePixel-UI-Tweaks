...
(function() {
        ...
        return {
            ...
            updateButtons: function() {
                ...
                potions.forEach(potion=>{
                    ...
                    getVar(potion, 0, 'int') ? useButton.style.color = 'white' : useButton.style.color = 'red'
                    sCBrewing().canBrew(potion) ? brewButton.style.color = 'white' : brewButton.style.color = 'red'
                })
                let combatLootPotionsAmount = document.getElementById('combat_loot_potion-label');
                combatLootPotionsAmount.textContent = getVar('combat_loot_potion_timer', 0, 'int') == 0 ?
                ...
                rainPotionsAmount.textContent = getVar('rain_potion_timer', 0, 'int') == 0 ?
    ...
    `;
                ...
                SCSMELTABLES.forEach((ore) => {
                    $(`itembox[data-item="${ore}"]`).on("contextmenu", (event) => {
                        if (
                            ...
                            if (primary || alt) {
                                sCMiningCrafting().quickSmelt(item, !primary);
            ...
            quickMining: function(item, alt) {
                ...
                singleOverride =
                    getThis.getConfig("autoSingleEnabled");
    ...
    const gathering = function() {
        window.SCLOOT_BAGS = $(`itembox[data-item^="gathering_loot_bag_"]`)
            .toArray()
    ...
    const farming = function() {
        ...
        window.SCPLANTABLES = $('itembox[data-item$="_seeds"]')
            .toArray()
            .map((el) => el.getAttribute("data-item"));
        ...
        return {
            ...
            initQuickBones: function() {
                SCBONEMEALABLE.forEach((item) => {
                    $(`itembox[data-item="${item}"]`).on("contextmenu", (event) => {
                        if (
                            getThis.getConfig("quickBoneRightClickEnabled")
                        ) {
                            const primary = sCActionType().primary(event);
                            const alt = sCActionType().alt(event);
                            if (primary || alt) {
                    ...
                    $(`itembox[data-item="${item}"]`).on("contextmenu", (event) => {
                        if (
                            ...
                            if (primary || alt) {
                                if (
                                    getThis.getConfig(
                                        "quickPlantHarvestRightClickEnabled"
                                    )
                                ) {
        ...
        return {
            initQuickCook: function() {
                SCCOOKABLES.forEach((item) => {
                    $(`itembox[data-item="${item}"]`).on("contextmenu", (event) => {
                        if (
                            getThis.getConfig("quickCookRightClickEnabled")
                        ) {
                            const primary = sCActionType().primary(event);
                            const alt = sCActionType().alt(event);
                            if (primary || alt) {
            ...
            initQuickEat: function() {
                SCEDIBLES.forEach((item) => {
                    $(`itembox[data-item="${item}"]`).on("contextmenu", (event) => {
                        if (
                            getThis.getConfig("quickEatRightClickEnabled")
                        ) {
                            const primary = sCActionType().primary(event);
                            const alt = sCActionType().alt(event);
                            if (primary || alt) {
    ...
    const fishing = function() {
        window.SCBOATS = $(`itembox[data-item$="_boat"], itembox[data-item$="_ship"]`)
            .toArray()
            .map((el) => el.getAttribute("data-item"));

        window.SCBAITS = $(`itembox[data-item$="bait"]`)
            .toArray()
            .map((el) => el.getAttribute("data-item"));
        ...
        return {
            ...
            initQuickBoat: function() {
                SCBOATS.forEach((item) => {
                    $(`itembox[data-item="${item}"]`).on("contextmenu", (event) => {
                        if (
                            getThis.getConfig("quickBoatRightClickEnabled")
                        ) {
                            const primary = sCActionType().primary(event);
                            const alt = sCActionType().alt(event);
                            if (primary || alt) {
            ...
            initQuickBait: function() {
                SCBAITS.forEach((item) => {
                    $(`itembox[data-item="${item}"]`).on("contextmenu", (event) => {
                        if (
                            getThis.getConfig("quickBaitRightClickEnabled")
                        ) {
                            const primary = sCActionType().primary(event);
                            const alt = sCActionType().alt(event);
                            if (primary || alt) {
    ...
    const invention = function() {
        window.SCGRINDABLE = $(`#panel-invention itembox[data-item^="blood_"][onclick^="Invention.clicks_limb"]`)
            .toArray()
            .map((el) => el.getAttribute("data-item"));
    ...
    const combat = function() {
        ...
        return {
            loadPresets: function(buttonNum) {
                ...
                let allPresets = JSON.parse(localStorage.getItem(SCUSERNAME + ".combat_presets")) || {};
...
`);
    ...
    `);
            ...
            quickFight: function(zoneId) {
                const confirm =
                      getThis.getConfig("quickFightConfirm");
                if (confirm) {
                    if (
                        sCCombat().confirm(
                            `FIGHT: ${zoneId
            ...
            initQuickExplode: function() {
                ...
                $(`itembox[data-item="large_tnt"]`).on("contextmenu", (event) => {
                    if (getThis.getConfig("quickExplosionEnabled")) {
                        ...
                        const alt = sCActionType().alt(event);
                        if (primary || alt) {
                            sCCombat().quickExplode("large_tnt");
                            event.stopPropagation();
                            event.preventDefault();
                            return false;
                        }
                ...
                $(`itembox[data-item="mega_bomb"]`).on("contextmenu", (event) => {
                    if (getThis.getConfig("quickExplosionEnabled")) {
                        ...
                        const alt = sCActionType().alt(event);
                        if (primary || alt) {
                            sCCombat().quickExplode("mega_bomb");
                            event.stopPropagation();
                            event.preventDefault();
                            return false;
                        }
            ...
            initPresets: function() {
	          ...
	          <br />
              <br />
              <img data-tooltip="Preset 6" id="in-combat-presets-icon-6" onclick="sCCombat().loadPresets(6)" class="combat-presets-combat-icon hover w30" src="https://d1xsc8x7nc5q8t.cloudfront.net/images/melee.png" />
              <img data-tooltip="Preset 7" id="in-combat-presets-icon-7" onclick="sCCombat().loadPresets(7)" class="combat-presets-combat-icon hover w30" src="https://d1xsc8x7nc5q8t.cloudfront.net/images/melee.png" />
    ...
    class SlapChopPlugin extends IdlePixelPlusPlugin {
        ...
        onConfigsChanged() {
            if(onLoginLoaded) {
                ...
                const slapchopQuickSmeltMining = document.getElementById("slapchop-quicksmelt-mining");
                const slapchopQuickSmeltCrafting = document.getElementById("slapchop-quicksmelt-crafting");
                ...
                if (getThis.getConfig("quickTreeNotificationHarvestEnabled")) {
                    notificationTreesReady.setAttribute('onClick', `sCWoodcutting.quickChop(); switch_panels('panel-woodcutting')`);
                } else {
        ...
        onVariableSet(key, valueBefore, valueAfter) {
            if(onLoginLoaded) {
                ...
                if(Globals.currentPanel == "panel-mining" || Globals.currentPanel == "panel-crafting") {
                    if([SCSMELTABLES, "oil", "charcoal", "lava", "dragon_fire", "stone_furnace", "bronze_furnace", "iron_furnace", "silver_furnace", "gold_furnace", "promethium_furnace", "titanium_furnace", "ancient_furnace","dragon_furnace"].includes(key)) {
                        sCMiningCrafting().updateQuickSmelt();
