...
(function() {
    ...
    class UITweaksPlugin extends IdlePixelPlusPlugin {
        constructor() {
            super("actlogtweaks", {
                about: {
                    name: `IdlePixel Activity Log Tweaks (ver: 0.7.11)`,
                    version: `0.7.11`,
                    author: `GodofNades`,
                    description: `Adds a new activity log to the top next to player count and introduces a new Activity Log modal.`
                },
            ...
            this.activeFilters = new Set(['Achievement', 'Brewing', 'Combat', 'Cooking', 'Crafting', 'Criptoe', 'Economy', 'Farming', 'Fishing', 'Gems', 'Geodes', 'New Loot', 'Woodcutting', 'XP', 'Uncategorized']);
        }
    ...
    `;
            ...
            document.getElementById('al-modal-base_window').addEventListener('click', function(event) {
        ...
        initFilters() {
            ...
            let categories = ['Achievement', 'Brewing', 'Combat', 'Cooking', 'Crafting', 'Criptoe', 'Economy', 'Farming', 'Fishing', 'Gems', 'Geodes', 'New Loot', 'Woodcutting', 'XP'];
            ...
            for (let i = 0; i < 3; i++) {
                ...
                const columnCategories = categories.slice(i * perColumn, (i + 1) * perColumn);
            ...
            activitySubheader.parentNode.insertBefore(filterContainer, activitySubheader.nextSibling);
        ...
        applyFilters() {
            ...
            rows.forEach((row, index) => {
                ...
                if (this.activeFilters.has(categoryCellText) || this.activeFilters.size === 0) {
                    // Show the row
                    row.style.display = '';
                } else {
                    // Hide the row
        ...
        zebraStripeRows() {
            const visibleRows = document.querySelectorAll('#al-table-table tbody tr:not([style*="display: none"])');
        ...
        onLogin() {
            ...
            activityLog = JSON.parse(localStorage.getItem(player + '.activityLog')) || [];
        ...
        convertUnixToReadable(unixTimestamp) {
            ...
            const hourFormatter = new Intl.DateTimeFormat(undefined, { hour: '2-digit', hour12: false });
            const minuteFormatter = new Intl.DateTimeFormat(undefined, { minute: '2-digit' });
            const secondFormatter = new Intl.DateTimeFormat(undefined, { second: '2-digit' });
        ...
        buildActivityLog(time, category, message, color ) {
            ...
            localStorage.setItem(player + '.activityLog', JSON.stringify(activityLog));
        ...
        displayActivityLog() {
            ...
            let activityLog = JSON.parse(localStorage.getItem(player + '.activityLog')) || [];
            ...
            activityLog.forEach(logEntry => {
                ...
                tdTimestamp.textContent = this.convertUnixToReadable(logEntry.timestamp);
        ...
        onVariableSet(key, valueBefore, valueAfter) {
            if(key === "researcher_points" && valueBefore && valueBefore != valueAfter) {
                let timestamp = this.formatDate();
                this.buildActivityLog(timestamp, "Criptoe", `Research Points gained: ${(valueAfter - valueBefore).toLocaleString()}`, "green");
            }
        }
